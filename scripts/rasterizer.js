...
service = server.listen(port, function(request, response) {
  ...
  var renderPage = function(pageUrl) {
    var page = new WebPage();
    var path = basePath + (request.headers.filename || (pageUrl.replace(new RegExp('https?://'), '').replace(/\//g, '.') + '.png'));
    ...
    page.onNavigationRequested = function(myurl, type, willNavigate, main) {
        if (main && pageUrl !== myurl) {
            pageUrl = myurl;
